...
def launch_ntSynt(*genomes, prefix="ntSynt", k=24, w=1000):
    ...
    cmd = f"ntSynt.py {genome_list} -k{k} -w {w} --prefix {prefix}"
    cmd = shlex.split(cmd)
...
def test_2_genomes():
    ...
    are_expected_blocks("celegans-A-ntSynt.merge_collinear.synteny_blocks.tsv",
                        "expected_result/celegans-A-ntSynt.merge_collinear.synteny_blocks.tsv")
...
def test_3_genomes():
    ...
    are_expected_blocks("celegans-A-B-ntSynt.merge_collinear.synteny_blocks.tsv",
                        "expected_result/celegans-A-B-ntSynt.merge_collinear.synteny_blocks.tsv")
