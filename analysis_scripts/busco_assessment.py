...
def extend_busco_list(buscos: list) -> None:
    ...
    present_buscos = {busco_id.id for busco_asm_list in buscos for busco_id in busco_asm_list.buscos}
    for busco_asm_list in buscos:
        for busco in reversed(busco_asm_list.start_reserve):
            if busco.id in present_buscos:
...
def filter_repeat(buscos_list: list) -> list:
    "Filter out BUSCOs that (after merging representative) are found more than once per assembly"
    dup_buscos = set()
